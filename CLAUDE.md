...
tests/
  ...
  test_server.py    boots the real server on a free port (-d :memory:), HTTP + WS lifecycle
...
- `serve()` in `__main__.py` loads `mktask.toml` via `mkio.config.load_config`, injects `version = __version__`, resolves `"__mkui__"` to `mkui.static_dir` and relative static dirs against the TOML's directory, then `create_app(cfg).run()`. A startup hook prints the banner.
...
- `test_ui_config.py` checks every dialog/button field against the op's `fields` + `key`; add the column to the op in `mktask.toml` before sending it from the UI.

## Versioning and release

`mktask/__init__.py` `__version__` is the single source of truth (hatch dynamic version). Release: bump it, `python -m pytest`, `python -m build`, `twine upload dist/*`, tag `vX.Y.Z`.
